...
TEMPLATES['ROCmValidationSuite'] = (
    (
        r"## (\(Unreleased\))? RVS (?P<lib_version>\d+\.\d+(?:\.\d+))?"
...
def ROCmValidationSuite_processor(data: ReleaseLib, template: str, _, __) -> bool:
    ...
    data.message = (
        f"ROCmValidationSuite for ROCm"
        f" {data.full_version}"
    )
...
PROCESSORS['ROCmValidationSuite'] = ROCmValidationSuite_processor
