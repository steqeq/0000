...
* Hang is observed with rocSPARSE tests: [Issue x]().
* AddressSanitizer instrumentation is incorrect for device global variables: [Issue x]().
* Dynamically loaded HIP runtime library references incorrect version of `hipDeviceGetProperties`
  API: [Issue x]().
* Memory access violations when running rocFFT-HMM: [Issue x]().
