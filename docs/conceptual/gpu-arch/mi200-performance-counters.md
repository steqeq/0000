...
This document lists and describes the hardware performance counters and the derived metrics
available on the AMD Instinct™ MI200 GPU. All hardware performance monitors, and the derived
performance metrics are accessible via AMD ROCm™ Profiler tool.

## MI200 performance counters list

```{note}
Preliminary validation of all MI200 performance counters is in progress. Those with “[*]” appended to
the names require further evaluation.
```
...
| `GRBM_TC_BUSY` | Cycles | Number of cycles any of the texture cache (TC) blocks (TCP/TCI/TCA/TCC) are busy |
...
The compute unit counters are further classified into instruction mix, MFMA operation counters, level
counters, wavefront counters, wavefront cycle counters, local data share counters, and others.
...
| `SQ_INSTS_VALU` | Instr | Number of VALU instructions including MFMA issued |
| `SQ_INSTS_VALU_ADD_F16` | Instr | Number of VALU F16 ADD/SUB instructions issued |
| `SQ_INSTS_VALU_MUL_F16` | Instr | Number of VALU F16 Multiply instructions issued |
| `SQ_INSTS_VALU_FMA_F16` | Instr | Number of VALU F16 FMA/MAD instructions issued |
| `SQ_INSTS_VALU_TRANS_F16` | Instr | Number of VALU F16 Transcendental instructions issued |
| `SQ_INSTS_VALU_ADD_F32` | Instr | Number of VALU F32 ADD/SUB instructions issued |
...
| `SQ_INSTS_VMEM_WR` | Instr | Number of VMEM Write instructions (including FLAT) issued                                  |
...
| Hardware counter | Unit  | Definition |
| :-------------------| :-----| :------------------- |
| `SQ_ACCUM_PREV` | Count | Accumulated counter sample value; accumulation takes place once every four cycles |
| `SQ_ACCUM_PREV_HIRES` | Count | Accumulated counter sample value; accumulation takes place once every cycle |
| `SQ_LEVEL_WAVES` | Waves | Number of inflight waves (calculate wave latency by dividing `SQ_ACCUM_PREV_HIRES` by `SQ_WAVE`) |
| `SQ_INST_LEVEL_VMEM` | Instr | Number of inflight VMEM (including FLAT) instructions (calculate VMEM latency by dividing `SQ_ACCUM_PREV_HIRES` by `SQ_INSTS_VMEM`) |
| `SQ_INST_LEVEL_SMEM` | Instr | Number of inflight SMEM instructions (calculate SMEM latency by dividing `SQ_ACCUM_PREV_HIRES` by `SQ_INSTS_SMEM_NORM`) |
| `SQ_INST_LEVEL_LDS`  | Instr | Number of inflight LDS (including FLAT) instructions (calculate LDS latency by dividing `SQ_ACCUM_PREV_HIRES` by `SQ_INSTS_LDS`) |
| `SQ_IFETCH_LEVEL` | Instr | Number of inflight instruction fetch requests from the cache (calculate instruction fetch latency by dividing `SQ_ACCUM_PREV_HIRES` by `SQ_IFETCH`) |

#### Wavefront counters

| Hardware Counter     | Unit  | Definition                                                        |
| :--------------------| :-----| ----------------------------------------------------------------: |
| `SQ_WAVES` | Waves | Number of wavefronts dispatch to SQs (includes new and restored wavefronts) |
...
| `SQC_ICACHE_MISSES_DUPLICATE`  | Count  | Number of duplicate `L1I` cache misses where the previous lookup miss on the same cache line is not fulfilled yet |
...
unit, vector L1D cache, and texture cache arbiter.
...
| `TCC_CC_REQ[n]` |Req | Number of CC requests, measured at the tag block (value range for n: [0-31]) |
| `TCC_RW_REQ[n]` |Req | Number of RW requests, measured at the tag block (value range for n: [0-31]) |
...
| `TCC_READ[n]` |Req | Number of L2 cache read requests; includes compressed reads but not metadata reads (value range for n: [0-31]) |
...
| `TCC_EA_WRREQ[n]` |Req | Number of 32-byte and 64-byte transactions going over the `TC_EA_wrreq` interface (atomics may travel over same interface and are generally classified as write requests); does not include probe commands (value range for n: [0-31]) |
...
| `TCC_EA_WR_UNCACHED_32B[n]` |Req | Number of 32-byte write/atomic going over the `TC_EA_wrreq` interface due to uncached traffic (CC mtypes can produce uncached requests, which are included in this); a 64-byte request is counted as 2 (value range for n: [0-31]) |
...
| `TCC_EA_WRREQ_LEVEL[n]` | Req | Accumulated number of EA write requests in flight, primarily intended to measure average EA write latency; average write latency = `TCC_PERF_SEL_EA_WRREQ_LEVEL`/`TCC_PERF_SEL_EA_WRREQ` (value range for n: [0-31]) |
...
| `TCC_EA_ATOMIC_LEVEL[n]` | Req | Accumulated number of EA atomic requests in flight, primarily intended to measure average EA atomic latency; average atomic latency = `TCC_PERF_SEL_EA_WRREQ_ATOMIC_LEVEL`/`TCC_PERF_SEL_EA_WRREQ_ATOMIC` (value range for n: [0-31]) |
...
| `TCC_EA_RDREQ_LEVEL[n]` | Req | The accumulated number of EA read requests in flight, primarily intended to measure average EA read latency; average read latency = `TCC_PERF_SEL_EA_RDREQ_LEVEL`/`TCC_PERF_SEL_EA_RDREQ` (value range for n: [0-31]) |
...
| `TCC_TAG_STALL[n]` | Cycles | Number of cycles the normal request pipeline in the tag is stalled for any reason--note that stalls are typically measured exactly at one point in the pipeline; however, with this counter, probes can stall the pipeline at a variety of places and there is no single point that can reasonably measure the total stalls accurately (value range for n: [0-31]) |
...
| :----------------| :------------------ |
| `ALUStalledByLDS` | Percentage of GPU time ALU units are stalled due to the LDS input queue being full or the output queue not being ready; reduce this by reducing the LDS bank conflicts or the number of LDS accesses if possible (value range: 0% (best) to 100% (worst)) |
...
| `GDSInsts` | Average number of GDS read/write instructions run per work item (affected by flow control) |
...
| `LDSBankConflict` | Percentage of GPU time LDS is stalled by bank conflicts (value range: 0% (best) to 100% (worst)) |
| `LDSInsts` | Average number of LDS read/write instructions run per work item (affected by flow control); excludes FLAT instructions that read from or write to LDS |
| `MemUnitBusy` | Percentage of GPU time the memory unit is active, which includes the stall time (`MemUnitStalled`); measured with all extra fetches and writes and any cache or memory effects taken into account (value range: 0% to 100% (fetch-bound)) |
| `MemUnitStalled` | Percentage of GPU time the memory unit is stalled; try reducing the number or size of fetches and writes if possible (value range: 0% (best) to 100% (worst)) |
| `MemWrites32B` | Total number of effective 32B write transactions to the memory |
| `SALUBusy` | Percentage of GPU time scalar ALU instructions are processed. Value range: 0% (worst) to 100% (best) |
...
| `TCC_EA_WRREQ_sum` | Total number of 32-byte and 64-byte transactions going over the TC_EA_wrreq interface, over all TCC instances (atomics may travel over the same interface and are generally classified as write requests); does not include probe commands |
...
| `TCC_EA_WR_UNCACHED_32B_sum` | Total number of 32-byte write/atomic going over the TC_EA_wrreq interface due to uncached traffic, over all TCC instances (note that CC mtypes can produce uncached requests, and those are included here); a 64-byte request is counted as 2 |
...
| `VALUBusy` | Percentage of GPU time vector ALU instructions are processed (value range: 0% (worst) to 100% (best)) |
| `VALUInsts` | Average number of vector ALU instructions executed per work item (affected by flow control) |
| `VALUUtilization`  | Percentage of active vector ALU threads in a wave; lower number can mean either more thread divergence in a wave or that the work-group size is not a multiple of 64 (value range: 0% (worst), 100% (best - no thread divergence)) |
...
| `WriteUnitStalled` | Percentage of GPU time the write unit is stalled (value range: 0% (best) to 100% (worst)) |
