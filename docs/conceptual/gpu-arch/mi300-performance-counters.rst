...
.. csv-table::
  ...
  "``TCC_EA0_WR_UNCACHED_32B_sum``", "Total Number of 32-byte write or atomic going over the ``TC_EA0_wrreq`` interface due to uncached traffic, over all texture cache per channel instances. Note that coherently cached mtypes can produce uncached requests, and those are included in this. A 64-byte request is counted as 2."
  ...
  "``TCC_WRITEBACK_sum``", "Total number of lines written back to the main memory including writebacks of dirty lines and uncached write or atomic requests, over all texture cache per channel instances."
  ...
  "``TCP_VOLATILE_sum``", "Total number of L1 volatile pixels or buffers from texture addressing unit, over all texture cache per pipe instances."
