...
* **Increase the amount of allocatable memory**
  ...
  By default, when using a device allocator via HIP, it is only possible to allocate 96GiB out of 
  a possible 128GiB of memory on the MI300A. This limitation does not affect host allocations.
  To increase the amount of available system memory, load the ``amdttm`` module with new values for
  ...
  .. note::
     ...
     per-APU limit of 4KiB pages, which is ``33554432``, by the number of APUs on the node. For a system with
     two APUs, the limit is ``33554432 x 2`` or ``67108864``.
