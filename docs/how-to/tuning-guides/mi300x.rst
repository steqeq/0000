...
This topic is a tuning guide for Instinct MI300X accelerators, which
built on the `AMD CDNA 3 architecture <https://rocm.docs.amd.com/en/latest/conceptual/gpu-arch/mi300.html>`_.
This topic can help you optimize performance.

.. include:: ./includes/mi300x-system.rst
...
This section lists some MI300X node settings and suggested values for improving
performance.

Performance critical environment variable
-----------------------------------------
...
For these settings, the ``env_check.sh`` script automates setting, resetting, and checking your
...
* As described in :ref:`mi300x-system-settings`, the environment
...
In an :ref:`MI300X architecture <mi300-node-level-arch-fig>`, there are
...
of the kernel. For more information, see `ROCr Debug
Agent documentation <rocr_debug_agent:index>`__. To summarize, the key points
include:
