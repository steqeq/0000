...
For a complete guide on how to install/manage/uninstall ROCm on Linux, refer to the
{doc}`Linux install guide <linux-install-docs:tutorial/install-overview>`. For verifying that the
installation was successful, refer to
{doc}`Linux install guide <linux-install-docs:how-to/native-install/install-overview>` and
