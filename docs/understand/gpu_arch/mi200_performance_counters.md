...
| `CPC_ME1_DC0_SPI_BUSY`          | Cycles | CPC ME1 Processor busy                              |
...
|`SPI_RA_REQ_NO_ALLOC_CSN`       | Cycles      | Arb cycles with CSn requests but no CSn allocation (need to multiply this value by 4) |
| `SPI_RA_RES_STALL_CSN`         | Cycles      | Arb stall cycles due to shortage of CSn pipeline slots (need to multiply this value by 4) |
...
| `SPI_RA_WVLIM_STALL_CSN[∗]`  | Cycles      | Number of clocks CSn is stalled due to WAVE_LIMIT            |
| `SPI_VWC_CSC_WR`               | Cycles      | Number of clocks taken to write CSC waves to VGPRs (need to multiply this value by 4) |
| `SPI_SWC_CSC_WR`               | Cycles      | Number of clocks taken to write CSC waves to SGPRs (need to multiply this value by 4) |
...
| `SQ_INSTS_VMEM_WR`        | Instr | Number of VMEM Write instructions issued. (including FLAT).                                   |
| `SQ_INSTS_VMEM_RD`        | Instr | Number of VMEM Read instructions issued. (including FLAT).  |
...
| `SQ_INSTS_SMEM_NORM`      | Instr | Number of SMEM instructions issued normalized to match `smem_level`(*2 load/store; *2 atomic; *2 memtime; *4 wb/inv). |
| `SQ_INSTS_FLAT`           | Instr | Number of FLAT instructions issued.                                     |
| `SQ_INSTS_FLAT_LDS_ONLY`  | Instr | Number of FLAT instructions issued that read/write only from/to LDS. Works only if EARLY_TA_DONE is enabled.       |
...
| `SQ_LEVEL_WAVES`      | Waves | Number of inflight waves. Set ACCUM_PREV for the next counter to use this.                |
| `SQ_INST_LEVEL_VMEM` | Instr | Number of inflight VMEM (including FLAT) instructions. Set next counter to ACCUM_PREV and divide by INSTS_VMEM for average latency.  |
| `SQ_INST_LEVEL_SMEM` | Instr | Number of inflight SMEM instructions (*2 load/store; *2 atomic; *2 memtime; *4 wb/inv). Set next counter to ACCUM_PREV and divide by INSTS_SMEM for average latency per SMEM request. Falls slightly short of the total requested latency because some fetches are divided into two requests that may finish at different times and this counter collects the average latency of the two.   |
| `SQ_INST_LEVEL_LDS`  | Instr | Number of inflight LDS (including FLAT) instructions. Set next counter to ACCUM_PREV and divide by INSTS_LDS for average latency.  |
...
| `SQ_INST_CYCLES_SALU`      | Cycles  | Number of quad-cycles spent to execute non-memory read scalar operations.    |
...
| `SQC_DCACHE_REQ`               | Req    | Number of sL1D cache requests. (post-bank-serialization).                                  |
...
| Hardware Counter                 | Unit   | Definition                                        |
| :--------------------------------| :------| ------------------------------------------------: |
| `TA_TA_BUSY[n]`                       | Cycles | TA busy cycles. `Perf_Windowing` is not supported for this counter. Value range for n: [0-15]. |
| `TA_TOTAL_WAVEFRONTS[n]`              | Instr  | Total number of wavefronts processed by TA. Value range for n: [0-15].       |
...
| `TA_ADDR_STALLED_BY_TC_CYCLES[n]`     | Cycles | Number of cycles TA address path is stalled by TC. `Perf_Windowing` is not supported for this counter. Value range for n: [0-15]. |
| `TA_DATA_STALLED_BY_TC_CYCLES[n]`            | Cycles | Number of cycles TA data path is stalled by TC.`Perf_Windowing` is not supported for this counter. Value range for n: [0-15].       |
| `TA_ADDR_STALLED_BY_TD_CYCLES[n]`  | Cycles | Number of cycles TA address path is stalled by TD. `Perf_Windowing` is not supported for this counter. Value range for n: [0-15].     |
...
| Hardware Counter         | Unit  | Definition                                           |
| :------------------------| :-----| ---------------------------------------------------: |
| `TD_TD_BUSY[n]`               | Cycle | TD busy cycles while it is processing or waiting for data. `Perf_Windowing` is not supported for this counter. Value range for n: [0-15].                            |
...
| `TCP_TCR_TCP_STALL_CYCLES[n]`            | Cycles | Number of cycles TCR stalls vL1D. Value range for n: [0-15].                           |7
...
| Hardware Counter | Unit   | Definition                                  |
| :----------------| :------| ------------------------------------------: |
| `TCA_CYCLE[n]`        | Cycles | Number of TCA cycles. Not windowable. Value range for n: [0-31].                               |
| `TCA_BUSY[n]`         | Cycles | Number of cycles TCA has a pending request. Not windowable. Value range for n: [0-31]. |
...
| Hardware Counter                 | Unit   | Definition                                                     |
| :--------------------------------| :------| -------------------------------------------------------------: |
| `TCC_CYCLE[n]`                        |Cycle   | Number of L2 cache free-running clocks. Not windowable. Value range for n: [0-31].               |
| `TCC_BUSY[n]`                         |Cycle   | Number of L2 cache busy cycles. Not windowable. Value range for n: [0-31].                                        |
...
| `TCC_PROBE[n]`                        |Req     | Number of probe requests. Not windowable. Value range for n: [0-31].  |
| `TCC_PROBE_ALL[n]`                 |Req     | Number of external probe requests with EA_TCC_preq_all== 1. Not windowable. Value range for n: [0-31].    |
...
| `TCC_EA_RDREQ_IO_CREDIT_STALL[n]`  | Cycles | Number of cycles there is a stall due to the read request interface running out of IO credits. Stalls occur irrespective of the need for a read to be performed or not. Value range for n: [0-31]. |
| `TCC_EA_RDREQ_GMI_CREDIT_STALL[n]` | Cycles | Number of cycles there is a stall due to the read request interface running out of GMI credits. Stalls occur irrespective of the need for a read to be performed or not. Value range for n: [0-31]. |
| `TCC_EA_RDREQ_DRAM_CREDIT_STALL[n]`   | Cycles | Number of cycles there is a stall due to the read request interface running out of DRAM credits. Stalls occur irrespective of the need for a read to be performed or not. Value range for n: [0-31]. |
...
| `FlatLDSInsts`     | Average number of FLAT instructions that read or write to LDS executed per work item (affected by flow control) |
