...
### Specialized Kernels
Clang might generate specialized kernels based on compiler options and OpenMP constructs. The following specialized kernels are supported:
- No-Loop
- Big-Jump-Loop
...
If the OpenMP construct has a reduction clause, the compiler may generate optimized code by utilizing efficient Xteam communication. New APIs for Xteam reduction are implemented in the device runtime and are automatically generated by clang.
