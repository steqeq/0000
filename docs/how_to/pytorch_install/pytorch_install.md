...
and choose the "ROCm" compute platform. {numref}`Installation-Matrix-from-Pytorch` is a matrix from pytorch.org that illustrates the installation compatibility between ROCm and the PyTorch build.
