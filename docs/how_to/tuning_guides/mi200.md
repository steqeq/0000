...
- The first block of the output shows the distance between the GPUs similar to
  the output of numactl command which applies to the NUMA domains of a system.
  The weight is a qualitative measure for the “distance” that the data travels
  to reach one GPU from another one. While the values do not carry a special
  (physical) meaning, the higher the value, the more hops are needed to reach
  the destination from the source GPU.
...
structure. For more details, the rocm-bandwidth-test can run benchmarks to show
the effective link bandwidth between the components of the system.
